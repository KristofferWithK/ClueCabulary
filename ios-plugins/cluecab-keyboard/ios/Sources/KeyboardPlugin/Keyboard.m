...
- (void)onKeyboardWillShow:(NSNotification *)notification
{
  ...
  /* CLUECAB FORK (of @capacitor/keyboard 8.0.5) — the whole diff is in this
   * method, and it forwards two numbers rather than changing any behaviour.
   *
   * Upstream reads the keyboard animation's duration here and uses it only to
   * schedule the resize, duration + 0.2s later — which is why the composer
   * used to arrive ~450ms after the keyboard: the height is known before the
   * animation starts and the page is told nothing about the timing, so the
   * ride in src/ui/nativeKeyboard.ts had to guess 250ms and a bezier.
   *
   * The delay itself is NOT trimmed, deliberately. setKeyboardHeight is
   * called before notifyListeners in this very method, and delay == 0 runs
   * the resize synchronously — the body would shrink before the JS listener
   * has frozen the board (--board-h / kb-up), reflowing it un-frozen for a
   * frame. With the ride on, the late shrink is invisible anyway: the
   * transform holds the dock at its final position until the handover.
   *
   * So: duration and curve join the event payload, the schedule stays
   * upstream's. `curve` is the raw UIView animation constant (7 in practice,
   * a private spring) — unused by the ride today, forwarded so the bezier can
   * be tuned from JS without another native build if filming demands it.
   */
