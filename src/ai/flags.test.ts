 ...
 * with no account attached. What makes it worth tapping is that Klaus is shown
 * the ones you flag — a flag that goes nowhere is a dead button.
 */
describe('flagged calls reach Klaus', () => {
...
/**
 * A flag carries a clue word, a Danish board word and Klaus's own sentence —
...
describe('flags carry nothing about anybody key', () => {
  it('the projection copies named fields rather than spreading the object', () => {
    const sneaky = {
      ...
      // legitimately, since Klaus is shown his OWN key there.
