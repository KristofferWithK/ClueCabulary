...
`merge-matrix.mjs` holds the four judgement calls, each argued in its header:
...
- **Merge rule** — `ceil(mean(opus, fable))`: the mean, ties to the higher
  ...
  which is why the merged matrix has more non-zero cells (864 before the
  floors) than either judge alone.
...
- **Conflict floor** — a `conflicts()` pair is raised to 2. The judges were
  told to ignore spelling and did: hund/hånd, kat/nat, bog/tog came back
  unrelated, which is right about meaning and wrong about play. 17 cells.
- **Concept floor** — a pair sharing a `concepts` tag is raised to 1, which is
  word-for-word what the brief defines a 1 as. 46 of the 421 same-concept
  pairs needed it, nearly all under the two widest tags, `nature` and `place`.

Both floors run after the honest judgement is recorded and list every cell they
move, with its pre-floor value, in the file's `meta.floored` — that list, not
the validator's green, is the cross-check's output. Measured size: 2,500 B
packed (two bits a cell, full square), 13.7 KB of JSON, **2.7 KB gzipped**.
Nine cities at this rate is ~25 KB gzipped, well under the ~100 KB the plan
budgeted.
