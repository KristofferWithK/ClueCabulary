...
- **E4** — Measure the engine, and decide — 2026-08-22 (PR #113). **The verdict is GO: E6 before E5, and E5 in halves** (written into `docs/clue-engine.md` "The verdict (E4, 2026-08-22)" and `docs/DECISIONS.md`, each with its reversal).
  ...
  **θ was re-measured on the surviving instrument, and it stays at 0.5.** The shipped bar was chosen on the row that just went void, so the whole sweep was re-run cross-model (`ENGINE_THETA_CROSS=1`). Board **cleared inside the tokens**, 200 games a cell — Opus→Fable 28.0 / **51.5** / 42.0 / 11.0 / 0.0 and Fable→Opus 36.5 / **59.5** / 41.0 / 9.5 / 0.5 at θ 0 / 0.5 / 1 / 1.5 / 2. Three things in that matter more than the answer: **the objective had to change with the instrument** — hits/number rises monotonically to 0.966 at θ=2.0, a bar at which *no board is ever finished*, so a clue engine tuned on hit rate picks the setting that never wins; **two of E3's three reasons were wrong** — θ=0 loses 23 points cross-model rather than eight, and above 0.5 the guesser *does* redeem some of the lost coverage (0.623 → 0.731), just not enough; and **the grid is exhaustive**, since `sim` returns multiples of 0.5 so a bar of 0.75 admits exactly what 1.0 admits. One comparison needed a bigger sample: at the suite's 40-game default 0.5-vs-1.0 **flips** in one direction (55.0 v 57.5), so it was settled at **400 games a cell** (48.5 v 39.8 and 53.8 v 38.5) and the committed pin defends only what survives every sample size. `LAST_CLUE_THETA` got the same treatment and a more honest answer: bars of 0 / 0.5 / 1.0 clear 47.8 / 48.5 / 47.0 and 53.5 / 53.8 / 52.0 — **the sweep cannot tell them apart**, so the constant is equal to θ for want of evidence rather than by choice, and the comment now says that. `engine.test.ts`'s old sweep is kept runnable and relabelled the retired instrument.
